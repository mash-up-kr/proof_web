...
function CompetitionBar({firstItemText, firstItemValue, secondItemText, secondItemValue}: CompetitionBarProps) {
  const firstRate = firstItemValue === 0 ? 0 : firstItemValue / (firstItemValue + secondItemValue) * 100;
  const secondRate = secondItemValue === 0 ? 0 : secondItemValue / (firstItemValue + secondItemValue) * 100;

  return (
      <Wrapper>
        <LeftBar isActive={firstItemValue >= secondItemValue} rate={firstRate}/>
        <RightBar isActive={secondItemValue >= firstItemValue} rate={secondRate}/>
        <ItemWrapper isFirst>
...
const LeftBar = styled.div<{isActive: boolean, rate: number}>`
  ...
  background: ${props => (props.isActive ? 'linear-gradient(270deg, #96B602 0%, rgba(186, 227, 72, 0) 100%)' : 'none')};
...
const RightBar = styled.div<{isActive: boolean, rate: number}>`
  ...
  background: ${props => (props.isActive ? 'linear-gradient(90deg, #6748E3 0%, rgba(103, 72, 227, 0) 100%)' : 'none')};
...
const ItemWrapper = styled.div<{isFirst?: boolean}>`
  z-index: 999;
  margin-right: ${props => props.isFirst ? 'auto' : 0}
`;
